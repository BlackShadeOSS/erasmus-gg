          ...
          filtered.map((r) => (
            ...
            >
              <summary className="flex items-center justify-between cursor-pointer">
                <div>
                  <div className="text-neutral-100 font-medium">
                    {r.content_type === "game"
                      ? gamesMap?.[r.content_id]?.title || r.content_id
                      : r.content_type === "vocabulary"
                      ? vocabMap?.[r.content_id]?.term_en || r.content_id
                      : r.content_id}
                  </div>
                ...
                <div className="flex items-center gap-3">
                  {r.content_type === "vocabulary" ? (
                    <div className="text-sm text-neutral-200">
                      Opanowanie:{" "}
                      <strong>
                        {vocabMap?.[r.content_id]?.mastery_level ?? 0}
                      </strong>
                    </div>
                  ) : (
                    <div className="text-sm text-neutral-200">
                      Wynik: <strong>{r.score ?? "-"}</strong>
                    </div>
                  )}
                </div>
              </summary>
              <div className="mt-3 text-neutral-300 text-sm space-y-2">
                ...
                {r.context && <div>Context: {JSON.stringify(r.context)}</div>}
                <div className="pt-2">
                  {r.content_type === "vocabulary" && (
                    <div className="flex items-center gap-2">
                      ...
                      <Progress
                        value={
                          ((vocabMap?.[r.content_id]?.mastery_level ?? 0) / 5) *
                          100
                        }
                        className="flex-1"
                      />

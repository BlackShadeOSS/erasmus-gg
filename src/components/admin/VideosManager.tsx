...
export default function VideosManager() {
  ...
  const fetchVideos = async (forceRefresh = false) => {
    try {
      ...
      const fetchPromise = fetch(`/api/admin/videos?${params}`);
      ...
      if (response.ok) {
        ...
        setHasError(true);
        showToast(data.error || "Błąd podczas pobierania filmów", "error");
      }
    } catch (error) {
      setHasError(true);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        showToast(
          editingVideo ? "Film został zaktualizowany" : "Film został utworzony",
          "success"
        );
  ...
  return (
    <div className="space-y-6">
      ...
      {loading ? (
        ...
        <div className="text-center py-8 text-neutral-400">Brak filmów</div>
      ) : (
        <Table>
          <TableHeader>
            <TableRow>
          ...
          <TableBody>
            {videos.map((video) => (
              <TableRow key={video.id}>
                <TableCell>
                  <div className="font-medium">{video.title_pl}</div>
      ...
      >
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              <Label htmlFor="title_en">Tytuł (angielski) *</Label>
              <Input
                id="title_en"
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <Label htmlFor="description_pl">Opis (polski)</Label>
              <Textarea
                id="description_pl"
            ...
            <div>
              <Label htmlFor="description_en">Opis (angielski)</Label>
              <Textarea
                id="description_en"
